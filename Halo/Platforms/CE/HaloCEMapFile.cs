...
{
  public class HaloCEMapFile : IMapFile
  {
    ...
    private int[] ceBitmapOffsets;
    private int[] ceSoundOffsets;
    private string[] ceSoundNames;
    private int[] ceLocOffsets;
    private string[] ceLocNames;
    private int uiiCount, siCount, bmiCount;
    ...
    private EndianReader loc;

    [Option("Bitmaps Cache", "Halo CE Shared Files", "bitmaps.map")]
    private static string ceBitmapsFile = "bitmaps.map";
    [Option("Sounds Cache", "Halo CE Shared Files", "sounds.map")]
    private static string ceSoundsFile = "sounds.map";
    [Option("Interface Cache", "Halo CE Shared Files", "loc.map")]
    private static string ceLocFile = "loc.map";
    ...
    public void Decompile(ILibrary library)
    {
      ...
      for (int x = 0; x < count; x++)
      {
        ...
        if (!library.FileExists(names[x] + '.' + tags[x].Xpe.Extension))
        {
          ...
          TagFile tf = new TagFile(stream.ToArray(), "Bungie", "Obtained from Halo CE.", tags[x].Xpe.FourCC, tags[x].Xpe.ParentFourCC, tags[x].Xpe.GrandparentFourCC, HaloCEGameDefinition.TagIdentifierStatic);
    ...
    public byte[] GetTag(string path)
    {
      ...
      TagFile tf = new TagFile(tag.GetStream(tags, false).ToArray(), "Bungie", "Obtained from Halo CE.", tag.Xpe.FourCC, tag.Xpe.ParentFourCC, tag.Xpe.GrandparentFourCC, HaloCEGameDefinition.TagIdentifierStatic);
      return tf.ToBytes();
    }
    ...
    public void Load(string filename)
    {
      ...
      string bitmapsFile = ceBitmapsFile;
      ...
      string soundsFile = ceSoundsFile;
      ...
      sounds.Position = 4;
      int siNameOffset = sounds.ReadInt32();
      int siOffset = sounds.ReadInt32();
      siCount = sounds.ReadInt32();
      ceSoundOffsets = new int[siCount];
      int[] ceSoundNameOffsets = new int[siCount];
      ceSoundNames = new string[siCount];
      for (int si = 0; si < siCount; si++)
      {
        sounds.Position = 12 * si + siOffset;
        ceSoundNameOffsets[si] = sounds.ReadInt32();
        sounds.Position += 4;
        ceSoundOffsets[si] = sounds.ReadInt32();
      }
      for (int si = 0; si < siCount; si++)
      {
        sounds.Position = ceSoundNameOffsets[si] + siNameOffset;
        ceSoundNames[si] = sounds.ReadString().Trim('\0');
      }

      // load the interface file
      string locFile = ceLocFile;
      if (locFile == "loc.map")
        locFile = Path.GetDirectoryName(filename) + '\\' + locFile;
      loc = new EndianReader(new FileStream(locFile, FileMode.Open, FileAccess.Read, FileShare.ReadWrite), ByteOrder.LittleEndian);
      loc.Position = 4;
      int uiiNameOffset = loc.ReadInt32();
      int uiiOffset = loc.ReadInt32();
      uiiCount = loc.ReadInt32();
      ceLocOffsets = new int[uiiCount];
      int[] ceLocNameOffsets = new int[uiiCount];
      ceLocNames = new string[uiiCount];
      for (int uii = 0; uii < uiiCount; uii++)
      {
        loc.Position = 12 * uii + uiiOffset;
        ceLocNameOffsets[uii] = loc.ReadInt32();
        loc.Position += 4;
        ceLocOffsets[uii] = loc.ReadInt32();
      }
      for (int uii = 0; uii < uiiCount; uii++)
      {
        loc.Position = ceLocNameOffsets[uii] + uiiNameOffset;
        ceLocNames[uii] = loc.ReadString().Trim('\0');
      }
          ...
          if ((index[x].flags & 1) > 0)
          {
            if (index[x].type == "bitm")
            {
              if (index[x].metaOffset < bmiCount)
                ...
                throw new HaloException("It should be noted that CE bitmaps flagged as external should not have an index ({0}) greater than the count ({1}) of such bitmaps.", index[x].metaOffset, bmiCount);
            ...
            if (index[x].type == "snd!")
            {
              ...
              if (soundIndex < 0)
                throw new HaloException("It should be noted that CE sounds flagged as external ({0}) should actually exist in the sounds cache.", names[x]);

              sounds.Position = ceSoundOffsets[soundIndex];
              tags[x] = new Tag(sounds, names[x], "snd!", ToUnsignedByReference(-sounds.Position), tags);*/
              continue;
            }
            ...
            {
              int locIndex = -1;
              for (int i = 0; i < uiiCount; i++)
              {
                if (names[x] == ceLocNames[i])
                {
                  locIndex = i;
                  break;
                }
              ...
              if (locIndex < 0)
                throw new HaloException("It should be noted that CE unicode strings flagged as external ({0}) should actually exist in the locs cache.", names[x]);

              loc.Position = ceLocOffsets[locIndex];
              tags[x] = new Tag(loc, names[x], "ustr", ToUnsignedByReference(-loc.Position), tags);
              continue;
            }
            ...
            if (index[x].type == "font")
            {
              ...
              if (locIndex < 0)
                throw new HaloException("It should be noted that CE fonts flagged as external ({0}) should actually exist in the locs cache.", names[x]);

              loc.Position = ceLocOffsets[locIndex];
              tags[x] = new Tag(loc, names[x], "font", ToUnsignedByReference(-loc.Position), tags);
              continue;
            }
            ...
            if (index[x].type == "hmt ")
            {
              ...
              if (locIndex < 0)
                throw new HaloException("It should be noted that CE HUD messages flagged as external ({0}) should actually exist in the locs cache.", names[x]);

              loc.Position = ceLocOffsets[locIndex];
              tags[x] = new Tag(loc, names[x], "hmt ", ToUnsignedByReference(-loc.Position), tags);
              continue;
            }

            throw new HaloException("The external flag was set on a type [{0}] that has no external handler.", index[x].type);
          }
        }
        Tag tag = null;
        uint bspMagic = 0;
    ...
    public void CompileCache(Stream output, ILibrary library)
    {
      ...
      // no indirect buffers in halo pc
      for (int x = 0; x < count; x++)
      {
        if (tags.GetClass(x) == "sbsp")
        {
          ...
          for (int y = 0; y < lmCount; y++)
          {
            ...
            for (int z = 0; z < matCount; z++)
            {
              ...
              // bsp vertices in halo pc are standard data, without special treatment
      ...
      for (int x = 0; x < count; x++)
      {
        ...
        if (tags.GetClass(x) == "bitm")
        {
          ...
          // we already wrote this, if necessary
      ...
      header.Write(MapBuildVersion.CE, false);
